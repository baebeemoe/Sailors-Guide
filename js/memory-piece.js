...
function renderMemoryDetail() {
  const hasMaxGrowth = Boolean(memoryPiece.maxGrowth && memoryPiece.maxStats);
  if (!hasMaxGrowth) showingMaxGrowth = false;

  const growth = showingMaxGrowth ? memoryPiece.maxGrowth : memoryPiece.normal;
  const stats = showingMaxGrowth ? memoryPiece.maxStats : memoryPiece.normalStats;
  ...
  detailRoot.innerHTML = `
      ...
      <div class="wrap memory-detail-grid">
        <div class="memory-detail-art">
          <div class="memory-card-frame ${showingMaxGrowth ? "max-growth-active" : ""}">
            ${memoryPiece.image ? `<img src="${memoryPiece.image}" alt="${memoryPiece.name}">` : `<div class="memory-placeholder large">◇</div>`}
          </div>
        </div>

        <div class="memory-info-panel">
          <div class="memory-banner">
            <div><p class="eyebrow">MEMORY PIECE</p><h1>${memoryPiece.name}</h1></div>
          ...
          <div class="stat-mode-label">${showingMaxGrowth ? "MAX GROWTH STATS" : "BASE STATS"}</div>
          <div class="base-stat-panel">
            <div><span>Basic ATK</span><strong>${stats.atk}</strong></div>
            <div><span>Basic DEF</span><strong>${stats.def}</strong></div>
            <div><span>Basic HP</span><strong>${stats.hp}</strong></div>
          </div>
          ...
          <div class="max-growth-control">
            <div>
              ...
              <small>${hasMaxGrowth ? (showingMaxGrowth ? "MAX Growth values enabled" : "Current growth values") : "MAX Growth data pending"}</small>
            </div>
            <button id="maxGrowthToggle" class="growth-toggle ${showingMaxGrowth ? "active" : ""}" aria-pressed="${showingMaxGrowth}" aria-label="Toggle MAX Growth" ${hasMaxGrowth ? "" : "disabled"}><span></span></button>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="wrap memory-notes">
      <div class="memory-note-card">
        ...
        <p>This Memory Piece's role-specific skill effect only applies when equipped by a <strong>${memoryPiece.role}</strong>.</p>
      </div>
      <div class="memory-note-card">
        <p class="eyebrow">GUIDE NOTES</p>
        <h2>${hasMaxGrowth ? "Growth Data Available" : "MAX Growth Pending"}</h2>
        <p>${hasMaxGrowth ? "Use the MAX Growth toggle above to compare its fully upgraded stats and effects." : "The confirmed base values are shown above. MAX Growth stats and upgraded skill values will be added once they are documented."}</p>
      </div>
    </section>
  `;

  const toggle = document.getElementById("maxGrowthToggle");
  if (hasMaxGrowth) toggle.addEventListener("click", () => {
...
if (detailMenuBtn && detailNav) detailMenuBtn.addEventListener("click", () => detailNav.classList.toggle("open"));
