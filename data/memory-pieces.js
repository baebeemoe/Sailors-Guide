  ...
  function render() {
    ...
    grid.innerHTML = filtered.map(piece => `
      <a class="memory-card" href="memory-piece.html?id=${encodeURIComponent(piece.id)}">
        <div class="memory-art">${piece.image ? `<img src="${piece.image}" alt="${piece.name}">` : `<div class="memory-placeholder">◇</div>`}<span class="rarity">${piece.rarity}</span></div>
        <div class="memory-card-body"><div class="memory-title-row"><div><h2>${piece.name}</h2><p>Memory Piece</p></div><span class="role-chip">${piece.role}</span></div><div class="memory-stats-mini"><span>ATK <strong>${piece.normalStats.atk}</strong></span><span>DEF <strong>${piece.normalStats.def}</strong></span><span>HP <strong>${piece.normalStats.hp}</strong></span></div><div class="badges"><span>${piece.role} Only</span><span>View Details →</span></div></div>
      </a>`).join("");
  }
  tabs.forEach(tab => tab.addEventListener("click", () => {tabs.forEach(x => x.classList.remove("active"));tab.classList.add("active");activeRole = tab.dataset.role;render();}));
  search.addEventListener("input", render);rarity.addEventListener("change", render);render();
}
